...
# 의도 분류 모델 모듈
class IntentModel:
    def __init__(self, proprocess, nermodel, customer=None):
        ...
        if customer != None:
            ...
            self.phone_num=customer.phone_num
            self.adr=customer.adr
            self.flag=customer.flag
            self.bag=customer.bag    
        else:
            self.name=None
            self.phone_num=None
            self.adr=None
        ...
        self.rec_words=["추천", "인기", "같이", "키즈", "가족", "커플", "적당", "괜찮", "맛있", "많이", "혼자", "혼밥", "비건", "베지테리언", "채식"]
        self.rec_words2=["어떤"]
        self.help_words=["문의", "민원", "상담원", "고객", "센터", "서비스","지원","멤버","환불", "비밀번호", "아이디", "회원"] #FAQ류 질문이 많으므로, 1:1대응되는 답변을 많이 준비해야함
        self.help_words2=["질문", "궁금", "물어보", "알리"] #FAQ류 질문이 많으므로, 1:1대응되는 답변을 많이 준비해야함
        self.event_words=["할인", "혜택", "프로모션", "이벤트", "행사"]
        ...
        self.vague_menu_name=['음료수', '탄산', '탄산음료', '라이스', '타코', '부리또', '세트메뉴', '화이타', '햄버거', '버거', '세트', '플레터']
        ...
        # self.vague_menu_name1 = ['음료수','음료', '탄산', '무알콜음료','탄산음료','셍맥','주류','술'] #음료메뉴 안내
        # self.vague_menu_name2 = ['라이스','밥','스프','수프'] #라이스$스프 카테고리 메뉴 안내
        # self.vague_menu_name3 = ['화이타', '파히타','스테이크','소고기','메인메뉴'] #화이타&스테이크 메뉴 안내
        # self.vague_menu_name4 = ['타코','부리또','브리또','콤보'] #타코 &부리또&콤보 메뉴 안내
        # self.vague_menu_name5 =['후식','디저트', '햄버거', '버거','감튀','수제버거'] #버거&디저트 메뉴 안내
    ...
    def predict_class(self, query):
        ...
        if 'QT' in tags and 'PS' in tags and '메뉴' in keywords: #몇명이서 먹을만한 메뉴 알려줘
            ...
            return "주문"
        elif class_check(self.bag, keywords) and class_check(self.ordercancel, keywords):
            return "주문취소"
...
# 의도 클래스 보조단어 출력
    def detailed_class_check(self, query):
        ...
        if 'QT' in tags and 'PS' in tags and '메뉴' in keywords: #몇명이서 먹을만한 메뉴 알려줘
            ...
            return return_word(self.rec_words, keywords)
        elif '먹' in keywords and '만' in keywords: #먹을만한 메뉴~
            return return_word(self.rec_words, keywords)
        elif class_check(self.reserv_words, keywords):
            return return_word(self.reserv_words, keywords)
        elif class_check(self.amb, keywords) and class_check(self.exact_menu_name, keywords): #구체적 메뉴명 +싶이면 주문으로, 아니면 추천으로 받음
            return None
        elif class_check(self.bag, keywords) and class_check(self.ordercancel, keywords):
            return None
            ...
            return return_word(self.branch, keywords)
        elif class_check(self.event_words, keywords):
            return None
        elif class_check(self.help_words, keywords):
